...
await new Promise(resolve=>setTimeout(resolve,900));ok('realtime přidání A → B',seen.some(e=>e.eventType==='INSERT'))
...
await new Promise(resolve=>setTimeout(resolve,900));ok('realtime koupení B → A/B',seen.some(e=>e.eventType==='UPDATE'&&e.new.status==='bought'))
...
await new Promise(resolve=>setTimeout(resolve,900));ok('realtime návrat a editace',seen.filter(e=>e.eventType==='UPDATE').length>=3)
...
await new Promise(resolve=>setTimeout(resolve,900));ok('realtime odstranění',seen.some(e=>e.new?.deleted_at))
